...
你（Codex／任何讀 AGENTS.md 的 agent）現在打開的是一位學員的工作區。
他剛上完「自動化工作流程設計」，把這個 repo clone 回家，
要把它改造成解決自己問題的東西。

## 先讀對應的那份指引，再開始做事

這個 repo 的助教邏輯寫在兩份檔案裡。**內容不重複寫在這裡** ——
下面兩份是唯一的正本，你要用的時候直接把整份讀進來：

| 他說的話 | 讀這份 |
|---|---|
| 「開始教我」「帶我做」「怎麼開始」「我想做一個自動化」 | `.claude/skills/workshop-guide/SKILL.md` |
| 「檢查一下」「我做好了嗎」「好了沒」「幫我看有沒有漏」 | `.claude/skills/blind-spot-check/SKILL.md` |

（路徑裡有 `.claude` 是因為 Claude Code 會自動掃那個目錄。
對你來說它就是一般的 markdown，直接讀就好。）
